    ...
    def add_item(self, item: Item) -> None:
        ...
        with psycopg.connect(self.dsn) as conn:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO item (project_id, type, text, source, created_at, updated_at, status)
                    VALUES (%s, %s, %s, 'cli', %s, %s, %s)
                    """,
                    (project_id, str(item.type), item.text, created_at, created_at, str(item.status)),
                )
            conn.commit()
