...
# Показать историю объединений (PostgreSQL)
...
Отклонение неподтвержденного предложения:
...
Безопасный rollback merge:

```bash
# показать историю объединений
python app/main.py list-merges

# откатить последний merge
python app/main.py undo-merge

# откатить merge по id
python app/main.py undo-merge <merge_id>
```

Что делает undo:
- восстанавливает текст целевой записи до состояния до merge
- возвращает source-элементы из `archived` в исходный статус и текст
- удаляет автоматически проставленные `duplicate_of` связи source -> target
- помечает merge как `can_undo = false`, чтобы избежать повторного неконсистентного отката
